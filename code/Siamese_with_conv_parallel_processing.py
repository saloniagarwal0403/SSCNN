#python Siamese_with_conv.py>Siamese_with_conv_loss_4.txt
import sklearn
import numpy as np
import pandas as pd
...
def get_features(df_train_X):
    ...
    for i in shuf[0:100]:
        ...
        train_features.extend(CreateInputFeatureMaps.get_model_predictions(filepath_i))
...
def permissible_pairs(X, Y, DAYS_DIFF, pca, n_pca_f, tensors_size, saving_folder):
    ...
    #     filepath_i = X_filenames.iloc[i]
    #     CreateInputFeatureMaps.create_tensors(filepath_i, pca, n_pca_f, tensors_size, saving_folder)
    for i in range(len(X)):
...
print("Screen Name: Siamese_with_conv_PCA_loss4")
EPOCHS = 10000
PATIENCE = 150
BATCH_SIZE = 32
DAYS_DIFF = 365
...
# for part in result:
#     print(part,'\n')
output_files_folder = os.path.join(r"/home","sxa171531","images","RMS","DataSummaries","FeatureMapGeneration","SSCNN","output_files")
for i in range(4,5):
    predictions = []
    ...
    train_pickle_filename = os.path.join(epoch_output_files_folder,'train.pickle')
    test_pickle_filename = os.path.join(epoch_output_files_folder,'test.pickle')
    ...
    loss_functions = [loss4, loss5, loss6]
    first_layer_neurons = [64,32,16,8]
    second_layer_neurons = [16,8,4]
    lrs = [0.001,0.0001]
    # lrs = [0.0001]
    results = []
    number_pca_features = [8,16,32,64,128,256,.7,.8]
    tensors_size = 42
    cvs_file_inputs = []
    for n_pca_f in number_pca_features:
        ...
        ### FOR EXTRACTING PCA FEATURES AND BUILDING VALID DATASETS
        gc.disable()
        ...
        for i_case in range(image_features_train_X1_pca.shape[0]):
            if random.random()<0.5:
                ...
                image_features_train_X1_pca_shuffled_new.append(image_features_train_X1_pca[i_case])
        ...
        for i_case in range(image_features_val_X1_pca.shape[0]):
            if random.random()<0.5:
                ...
                image_features_val_X1_pca_shuffled_new.append(image_features_val_X1_pca[i_case])
        ...
        for i_case in range(image_features_test_X1_pca.shape[0]):
            if random.random()<0.5:
                ...
                image_features_test_X1_pca_shuffled_new.append(image_features_test_X1_pca[i_case])
        ...
        # image_features_train_X1_pca = CreateInputFeatureMaps.pca_transform_tensors(pca, n_pca_f, 42, 42, image_features_train_X1)
        # image_features_train_X2_pca = CreateInputFeatureMaps.pca_transform_tensors(pca, n_pca_f, 42, 42, image_features_train_X2)
        # image_features_val_X1_pca = CreateInputFeatureMaps.pca_transform_tensors(pca, n_pca_f, 42, 42, image_features_val_X1)
        # image_features_val_X2_pca = CreateInputFeatureMaps.pca_transform_tensors(pca, n_pca_f, 42, 42, image_features_val_X2)
        # image_features_test_X1_pca = CreateInputFeatureMaps.pca_transform_tensors(pca, n_pca_f, 42, 42, image_features_test_X1)
        # image_features_test_X2_pca = CreateInputFeatureMaps.pca_transform_tensors(pca, n_pca_f, 42, 42, image_features_test_X2)
